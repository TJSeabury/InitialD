...
final class AggregatorCss
{
	...
	* @return array
	*/
	public static function getFiles( string $readPath, bool $pathOnly ): array
	{
		$r = array();
	...
	* @return string
	*/
	public static function agg( array $files ): string
	{
		return implode( "\n\n", $files );
	}
	...
	* @return string
	*/
	public static function minify( string $c ): string
	{
		return str_replace( 
			array( ' {', ': ', ', ', ' >', '> ', ' > ', ' !', '( ', ' )' ), 
			array( '{', ':', ',', '>', '>', '>', '!', '(', ')' ), 
	...
	* @return string
	*/
	private function get_file_output( $file ): string
	{
		ob_start();
		include( $file );
		return ob_get_clean();
